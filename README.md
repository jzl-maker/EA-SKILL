...
```
[scope]  ⚠️ 占空比在窗口内变化：逐周期 20.0% → 79.5%（极差 59.5 个百分点）
             上面报的 duty_pct=49.75% 只是整段窗口的均值      →  退出码 2
```
...
| 测出一个数，但不知道它可不可信 | 削顶/平线/不等周期/**占空比在变**一律标注 + 退出码 2，不静默给均值 |
...
- 🎯 **不给会误导判断的数**：数据不可信时明说而不是硬报一个值——削顶/平线/噪声边沿/不等周期/占空比在窗口内变化，全部进 `warnings` 并置退出码 `2`，按 rc 分支就不会把"取到但不可信"当成成功
- 🧪 **无硬件回归测试**：199 条用例驱动全部工具脚本，合成数据 + 假可执行文件，不接任何仪器也能跑（`tests/`）
...
|------|------|------|
| 烧录 | ST-Link / DAP | ✅ |
| 波形抓取/测量 | Rigol DS1074Z / DS2302A | ✅ |
| 深内存抓取（`--mode RAW`） | Rigol DS2302A（140 万点 @2 GSa/s） | ✅ |
| CSV 解析 | 正点原子 DS100 | ✅ |
| SVD 寄存器地图 | N32G4FR | ✅ |
...
├── tests/            # 无硬件回归测试（199 条，见 tests/README.md）
└── mcp-servers/      # 串口 MCP
```
