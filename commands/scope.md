...
**退出码**：`0`=正常 / `1`=失败（含参数错误、通道名写错、本机没这个通道、文件不存在、连不上）/ `2`=取到数据但不可信（削顶、平线、噪声、不等周期、**占空比在窗口内变化**、窗口内周期数不足、某项测不出）。**Rigol 实时、`--parse-tmc`、DS100 三条路径同一套语义**——解析平线文件也回 2。按 rc 分支时不要把 2 当失败丢弃——数据在，只是需要看 `warnings`。
...
DS100 路径的 `sample_domain` 与实时路径是**同一套判据**（`flat`/`irregular`/`period_cv`/`duty_irregular`/`duty_spread_pct`/`periods`），退出码也同一套：平线 → rc=2，窗口内不足 2 个完整周期 → rc=2 并说明占空比不可信。**不要只看 `freq_hz` 有没有值就下结论**——平线时它同样是 `null`，但"测到 0 V"和"没测到"是两件事。
...
| `--parse-tmc <file>` | 是（动作） | 解析原始 TMC 块文件（自测）。前导三元组由 `--yref/--yinc/--yor` 给，缺省 `127 / vdiv÷25 / 25×--voffs`——与在线路径**同一个公式**。同样输出 `warnings` 并按 0/2 退出码返回 |
...
| `--save <file.csv>` | 否 | 波形 CSV（time_s,voltage_v 两列）。**给人看的，不要 Read 进上下文**（见"读取纪律"） |
| `--plot <file.png>` | 否 | 波形图 png。**深内存下就是一块实心色块**，要出图先减小 `--points` |
...
`duty_pct` 是**整段窗口的平均值**，它会把"在变"的占空比压成一个数。三件配套字段
才是"稳不稳"的答案：

| 字段 | 含义 |
|------|------|
| `duty_min_pct` / `duty_max_pct` | 逐周期占空比的最小 / 最大值 |
| `duty_spread_pct` | 极差（max−min，百分点）——**判据用的是它** |
| `duty_cv` | 逐周期占空比的变异系数（std/mean），描述总体离散度 |

极差超过 `DUTY_SPREAD_PCT = 2.0` 个百分点就置 `duty_irregular`，并给一条警告、
把退出码置 2。合成波形实测（1000 个周期，5 kHz）：

| 波形 | `duty_pct` | min~max | 极差 | `duty_cv` | 判定 |
|------|-----------|---------|------|-----------|------|
| 20%→80% 线性扫 | 49.75% | 20.0~79.5 | 59.5 | 0.348 | ⚠️ 变化 |
| 丢 1 个脉冲 | 49.95% | 25.0~50.0 | 25.0 | 0.0158 | ⚠️ 变化 |
| 稳定 50% 方波 | 50.0% | 50.0~50.0 | 0.0 | 0.0 | ✅ 正常 |
| 稳定 49.5% 方波 | 49.5% | 49.5~49.5 | 0.0 | 0.0 | ✅ 正常 |

**为什么判据用极差而不是 CV**：看第 2 行——丢 1 个脉冲的 `duty_cv=0.0158`，远低于
`IRREGULAR_CV=0.15`，光看变异系数会判成"正常"；极差则直接掉到 25 个百分点。
CV 会被**少数周期异常**稀释，极差不会。

同理，`period_cv` 只在**周期抖动**方向上敏感：第 1 行占空比扫了 60 个百分点，
`period_cv` 仍然是 `0.0`（周期确实恒定），单看它 + `duty_pct=49.75%` 会读成
"一路稳定的 50% 方波"。**PWM 调光 / 软启动 / 闭环调节的波形全在这个盲区里**，
这类信号务必看 `duty_min_pct`~`duty_max_pct`。

要某个时刻的占空比，缩窄窗口（减小 `--points` 或调小时基）或改用 `--measure`
读仪器单周期结果——平均值本身不会告诉你它扫到哪里。

### 读取纪律：别把数据文件读进上下文

工具吐出的 stdout/stderr 与数据量**无关**，稳定在 650~1500 字符（实测 280 万点
→ 684 字符）。真正的 token 陷阱是**人去 Read 那些落盘文件**：

| 文件 | 实测 | 一次 Read 的代价 |
|------|------|------------------|
| 280 万点 CSV (`--save`) | 136.2 MB / 280 万行 / 每行 50 字符 | Read 上限 2000 行 ≈ **3.1 万 token**，只看到 **0.07%** |
| 同上，整个文件 | — | ≈ 4375 万 token（不可行） |

所以：

- **要看数据只用 `--json`**，不要 Read `--save` 出来的 CSV。CSV / PNG 是给人看的
  产物；JSON 才是给 AI 的。三者是同一份内存数组的三个兄弟，**不存在"CSV 转 JSON"
  这一步**——降维是单向的、有损的，波形容貌 / 脉宽分布 / 抖动分布 / 非周期事件 /
  趋势漂移 / 多通道相位关系全部不进 JSON。要这些信息，用 `--plot` 出图让**人**看，
  或者缩窄窗口重新采一段。
- **不要为了确认参数去跑 `--help`**（3410 字符）。参数表就在本文件里。
- 深内存（`--points` 上万）**不要配 `--plot`**：280 万点画进 1200 px 的图是每像素
  2333 个采样点，出来是一整块实心色块，没有任何信息。要看波形就减小 `--points`
  把窗口限定到关心的那几十个周期。
...
5. 输出        采样率/探头倍率（自动提取）+ 频率/周期/占空比/幅值 + 波形图
              **看 rc 与 warnings**：rc=2 表示波形是平线、周期数不足、或占空比在文件内
              变化，不是"成功"
```
...
这个数太小（<2）时占空比不可信。另外 `duty_pct` 是整段窗口的平均值，**占空比在文件内
变化时它不说明问题**，要看 `duty_min_pct`~`duty_max_pct`（见"占空比是点估计"一节）。
...
16b. **`period_cv` 管不了占空比**：上一条的变异系数只反映**周期**是否整齐，占空比怎么变它都可能是 `0.0`。实测合成波形——占空比 20%→80% 线性扫时周期确实恒定，`period_cv=0.0`、`irregular=false`，而 `duty_pct=49.75%` 读起来就是一个稳定 50% 方波，警告一条没有。现在逐周期算占空比并置 `duty_min_pct`/`duty_max_pct`/`duty_spread_pct`/`duty_cv`，极差超 `2.0` 个百分点即 `duty_irregular=true` + 警告 + rc=2。**判据用极差不是 CV**：丢 1 个脉冲的 `duty_cv` 只有 0.0158（低于 `IRREGULAR_CV=0.15`，会被判成正常），极差直接掉到 25 个百分点。详见"占空比是点估计"一节。
...
19. **`--parse-tmc` 也走 0/2 退出码契约**：这条路径以前**一条警告都没有、也永远返回 0**——削顶只在人类可读输出里顺带提一句，平线/不等周期/噪声边沿全部当成正常结果返回。它正是无硬件自测用的那条路径，最不该对数据质量沉默。现在它与实时路径、DS100 路径共用同一套判据（`_offline_warnings`），也返回 `warnings` 数组与 rc=2。
20. **`--json` 的改道机制在 `common.py`**：`diagnostics_to_stderr(enabled)` 把 `print` 改道 stderr，`emit_json` 则写模块导入期抓到的真 stdout，两者配套才成立。`scope.py` 和 `logic_analyzer.py` 共用它——**必须在任何诊断输出之前套上**（这两个脚本都在 `emit_json` 之前就已经 print 了设备信息/进度/产物清单，晚套等于没套）。调用处写成 `with diagnostics_to_stderr(args.json):`，`enabled=False` 时原样放行。
...
| 占空比看着正常但无意义 | 占空比在窗口内变化（PWM 调光/软启动/闭环），`duty_pct` 只是均值且 `period_cv` 抓不到 | 看 `sample_domain.duty_min_pct`~`duty_max_pct` 与 `duty_irregular`（见注意事项 16b） |
| `--parse-tmc` 平线文件报"成功" | 老版本该路径没有 warnings、永远 rc=0 | 已修复：与另外两条路径共用判据 → rc=2 + warnings（见注意事项 19） |
| AI 读取 `--save` 的 CSV 烧掉几万 token | CSV/PNG 是给人看的；降维是单向的，CSV 不会"转成"JSON | 看数据用 `--json`，看图用 `--plot`（见"读取纪律"） |
